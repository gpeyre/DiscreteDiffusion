...
Q^{\mathrm B}_{j,i}=a_j P_{j,i}\, b_i^+.
\end{equation}
In matrix form,
\begin{equation}
\label{eq:bayes-diag}
...
\end{proposition}
\begin{proof}
The component formula is exactly Bayes rule when $b_i>0$, and the pseudo-inverse convention sets the corresponding row to zero when $b_i=0$. Since $b=P^\top a$, the latter case implies
\[
b_i=\sum_j a_j P_{j,i}=0,
\]
hence $a_j P_{j,i}=0$ for every $j$, so the formula is still consistent.
...
\diag(b)^+ b.
\]
For every $i$ such that $b_i>0$, the $i$-th entry equals $1$. If $b_i=0$, then the $i$-th row of $Q^{\mathrm B}$ is identically zero and this row is never visited under the marginal $b$; equivalently, the conditional law on that null-mass event is immaterial. On the support of $b$, $Q^{\mathrm B}$ is therefore stochastic.
...
Let $Q_\theta^t(j,i)$ be a neural network parameterization such that, for each $(t,i)$,
\[
...
\[
Q_\star^t(j,i)\coloneqq \mathbb P(X^t=j\mid X^{t+1}=i,\,t),
\]
...
L(\theta)=C+\sum_{t=0}^{\infty}\lambda(t)\sum_{i=1}^n h_i^{t+1}\,
\KL\!\big(Q_\star^t(i,\cdot)\,\|\,Q_\theta^t(i,\cdot)\big),
\end{equation}
where
\[
C=\sum_{t=0}^{\infty}\lambda(t)\sum_{i=1}^n h_i^{t+1}\,H\!\big(Q_\star^t(i,\cdot)\big)
\]
...
\[
\sum_{t=0}^{\infty}\lambda(t)\sum_{i=1}^n h_i^{t+1}\,
\KL\!\big(Q_\star^t(i,\cdot)\,\|\,Q_\theta^t(i,\cdot)\big).
\]
...
\end{itemize}
Then any limit point of $Q_{\theta_N}$ minimizes $L$ and agrees with $Q_\star^t(i,\cdot)$ for all $(t,i)$ such that $\lambda(t)\,h_i^{t+1}>0$.
...
\[
Q_{j,i}^t=\frac{h_j^t P_{j,i}}{h_i^{t+1}}
\]
...
\]
For $x=x_i$ and $y=x_j$, define the piecewise-constant reverse density
\[
...
\end{equation}
Equivalently, when $x=x_i$ and $y=x_j$, one has $Q_\theta^t(x_i,x_j)=Q_{\theta,j,i}^t$.
